...
- 隔离（强制，2026-08-29 起）：集成测试**必须**通过 `TEST_DATABASE_URL` 使用独立测试库；`tests/conftest.py` 写死校验——缺少该变量或指向开发库（与 `DATABASE_URL` 同库）时直接拒绝运行，无任何豁免开关（`ALLOW_DESTRUCTIVE_TEST_DB` 已永久删除）。Redis 使用独立 db（编号通过测试环境变量配置，与开发缓存隔离）或 fakeredis。共享开发库对测试只读。
- 命名：`test_<场景>_<期望>`。
