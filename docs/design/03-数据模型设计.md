...
| category | VARCHAR(32) | NOT NULL | project/technology/contract/general（管理/技术/合同条款/通用四维度） |
...
### 3.12 sys_contract — 合同表

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| id / user_id | BIGSERIAL / BIGINT | PK / 所属用户 |
| file_name / file_path / file_ext / file_size | VARCHAR/INTEGER | 文件元数据 |
| text_content / total_chars | TEXT / INTEGER | 解析后纯文本 |
| status | SMALLINT | 1=完成，2=失败 |
| risk_count / high_count / medium_count / low_count | INTEGER | 风险统计 |
| created_at/updated_at/deleted_at/created_by/updated_by | — | 审计 |

### 3.13 sys_contract_risk — 合同风险结果表

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| id / contract_id / user_id | BIGINT | 主键/合同/用户 |
| rule_code / rule_name / category / severity | VARCHAR | 规则快照 |
| matched_keywords / snippet | JSONB / TEXT | 命中关键词/上下文 |
| description / suggestion / sort_order | TEXT/INTEGER | 规则快照 |
| created_at | TIMESTAMPTZ | |

详细设计见《11-合同风险识别核心功能设计》。
