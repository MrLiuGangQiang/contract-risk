...
| 变量 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- |
| `DATABASE_URL` | 推荐 | 空 | 聚合连接串（asyncpg），如 `postgresql+asyncpg://user:pass@host:5432/db`；为空时按 `POSTGRES_*` 拆分项组装（兼容旧写法） |
| `REDIS_URL` | 推荐 | 空 | 聚合连接串，如 `redis://:pass@host:6379/0`；为空时按 `REDIS_*` 拆分项组装 |
...
> 业务配置（钉钉等）仍由超管在 `sys_config` 维护，不占环境变量；聚合 URL 与拆分项二选一，聚合优先。
